...
const dom = {
  activeTasksContainer: document.getElementById("active-tasks"),
  inprogressTasksContainer: document.getElementById("inprogress-tasks"),
  ...
  suggestionsDone: document.getElementById("suggestions-done"),
...
async function createTaskOnServer(taskData) {
  ...
  try {
    ...
    return {
      ...
      status: result.status || taskData.status || "active",
      username: result.username || taskData.username || authStore.currentUser,
    };
  } catch (error) {
    showToast("Could not save task");
    return null;
...
async function updateTaskOnServer(taskId, taskData) {
  const token = Symbol();
  authStore.taskRequestTokens[taskId] = token;
  try {
    const response = await safeFetch(
    ...
    if (authStore.taskRequestTokens[taskId] !== token) return false;
    ...
    if (authStore.taskRequestTokens[taskId] === token) {
      delete authStore.taskRequestTokens[taskId];
    }
  }
}
...
async function safeFetch(url, options = {}, abortKey) {
  const signal =
    options.signal ||
    (abortKey ? createAbortController(abortKey).signal : undefined);
  try {
...
async function loadTasksFromAPI() {
  ...
  try {
    const query = new URLSearchParams({ username: currentUser });
    const response = await safeFetch(
      `${Api.base}${Api.tasks}?${query}`,
      { signal: authStore.abortControllers.tasks.signal },
      "tasks",
    );
    if (!response) return;
    if (
    ...
    state.tasks = serverTasks
      .filter((task) => task.username === currentUser)
      .map((task) => ({
        id: String(task.id),
        title: task.title || "",
        description: task.description || "",
        status: validateStatus(task.status) ? task.status : "active",
        username: task.username || currentUser,
    ...
    if (error.name !== "AbortError") {
      showToast("Could not load tasks from server");
    }
  } finally {
    showLoadingIndicator(false);
...
async function handleLogin() {
  ...
  try {
    ...
    if (!user) {
      dom.loginError.textContent = "Account not found";
      return;
    }
...
function validatePassword(password, confirm) {
  ...
  document.getElementById("rule-capital").classList.toggle("valid", hasCapital);
  document.getElementById("rule-match").classList.toggle("valid", hasMatch);

  return hasLength && hasCapital && hasMatch;
}
...
function setupSuggestionsPanel() {
  dom.suggestionItems.forEach((item) => {
    ...
    button.addEventListener(
      "click",
      withLocalActionGuard(button, async () => {
        if (item.classList.contains("added")) return;
        const title = item.dataset.title;
        const added = await addSuggestionTask(title);
        if (added) {
